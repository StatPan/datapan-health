...
policy key and version. A credential scope is an opaque, non-secret grouping
identifier. It prevents observations made with different credential contexts
from being combined and must never contain a credential ID, fingerprint, hash,
or value. Response bodies, rows, query values, provider prose, and full request
URLs are not part of this contract.
...
observations, mixed policies, mixed credentials, and cross-dependency evidence
